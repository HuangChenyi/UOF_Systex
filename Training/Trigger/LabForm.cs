...
namespace Training
{
    ...
    {
        public LabForm(string formVersionId, UrgentLevel Urgentlevel, string account, string userGuid, string userName)
        {
            ...
            foreach (var Propertie in this.Fields.GetType().GetProperties())
            {
                ...
                switch (field.FieldType)
                {
                    case FieldType.optionalField:
                        ...
                        FieldItemXE.Add(new XAttribute("realValue", field.RealValue));
                        FieldItemXE.Add(new XAttribute("ConditionValue", field.RealValue));

                        try
                        {

                            FieldItemXE.Add(XElement.Parse(optionalField.FieldValue));
                        }
                        catch
                        {
                            FieldItemXE.Value = optionalField.FieldValue;
                        }
                        break;
                                ...
                                foreach (var gridPropertie in row.GetType().GetProperties())
                                {
                                    ...
                                    switch (gridField.FieldType)
                                    {
                                        case FieldType.fileButton:
                                            ...
                                            cellElement.Add(new XAttribute("IsNeedTransfer", gridFileField.IsNeedTransfer.ToString()));
                                            cellElement.Add(new XAttribute("IsDeleteTemp", gridFileField.IsDeleteTemp.ToString()));
                                            cellElement.Add(new XAttribute("fieldValue", ""));
                                            if (gridFileField.FileAttach.Count > 0)
                                            {

                                                cellElement.Add(new XAttribute("fillerName", this.UserName));
                                                cellElement.Add(new XAttribute("fillerUserGuid", this.UserGuid));
                                                cellElement.Add(new XAttribute("fillerAccount", this.Account));
                                                cellElement.Add(new XAttribute("fillSiteId", ""));
    ...
    public class FieldList
    {
        ...
        public OptionalField Field_A01 { get; set; } = new OptionalField() { FieldId = "A01" };

        public Field Field_type { get; set; } = new Field() { FieldType = FieldType.radioButton, FieldId="type" };

        public Field Field_item { get; set; } = new Field() { FieldType = FieldType.singleLineText, FieldId="item" };

        public Field Field_amount { get; set; } = new Field() { FieldType = FieldType.numberText, FieldId="amount" };
